...
import {
  LayoutDashboard, FileText, GraduationCap, CheckSquare,
  Shield, FolderOpen, LogOut, ShieldAlert, ChevronRight
} from 'lucide-react'
...
const nav = [
  ...
  { to: '/pops', icon: FileText, label: 'POPs', badge: 1 },
  { to: '/treinamentos', icon: GraduationCap, label: 'Treinamentos', badge: 3 },
  { to: '/obrigacoes', icon: CheckSquare, label: 'Obrigações', badge: 2 },
  ...
  return (
    <aside className="w-56 bg-white border-r border-gray-100 flex flex-col h-screen sticky top-0 flex-shrink-0">
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        ...
        {nav.map(({ to, icon: Icon, label, badge, highlight }) => (
          <NavLink
            key={to}
            to={to}
          ...
          >
            ...
            {badge && (
              <span className="bg-red-500 text-white text-[10px] font-semibold w-4 h-4 rounded-full flex items-center justify-center">
                {badge}
              </span>
            )}
          </NavLink>
      ...
      <div className="px-3 py-3 border-t border-gray-100">
        <div className="flex items-center gap-2.5 p-2 rounded-lg bg-gray-50 mb-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-gray-900 truncate">{user?.clinica}</p>
