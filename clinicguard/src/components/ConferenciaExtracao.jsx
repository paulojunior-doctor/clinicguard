...
function normalizarPeriodicidade(valor) {
  ...
  const mapa = {
    'mensal': 'Mensal', 'quinzenal': 'Quinzenal', 'trimestral': 'Trimestral',
...
export default function ConferenciaExtracao({ open, loading, error, resultado, fileName, onClose, onConfirm }) {
  ...
  useEffect(() => {
    if (resultado?.extraction) {
      const iniciais = {}
      CAMPOS_MANUAL.forEach(({ chaveExtracao, chaveManual }) => {
        let v = resultado.extraction[chaveExtracao]?.value ?? ''
        if (chaveManual === 'periodicidade_pragas') v = normalizarPeriodicidade(v)
  ...
  const naoRelacionado = resultado?.classification === 'documento_nao_relacionado'
      ...
      <div style={ov.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={ov.body}>
          ...
          {!loading && !error && naoRelacionado && (
            <div style={ov.centerMsg}>
              <p style={{ fontWeight: 500 }}>Documento não reconhecido como certificado/laudo de controle de pragas.</p>
          ...
          {!loading && !error && resultado && !naoRelacionado && (
            <>
              ...
              {CAMPOS_CONTEXTO.map(({ chave, label }) => {
                const item = resultado.extraction?.[chave]
                if (!item?.value) return null
                return (
                  <div key={chave} style={ov.contextoRow}>
                    <span style={ov.contextoLabel}>{label}:</span> {typeof item.value === 'string' ? item.value : JSON.stringify(item.value)}
                  </div>
                )
              })}
              ...
              {CAMPOS_MANUAL.map(({ chaveExtracao, chaveManual, label, tipo }) => {
              ...
              {!campos.proxima_dedetizacao && !campos.periodicidade_pragas && (
                <div style={ov.warnBox}>
                  ⚠️ Próxima intervenção não identificada. O documento comprova a realização do serviço, mas não informa claramente a próxima intervenção ou periodicidade. Você pode preencher manualmente acima, ou confirmar assim mesmo — a obrigação de dedetização não será criada automaticamente sem essa informação.
                </div>
              )}
            </>
          )}
        ...
        <div style={ov.footer}>
          ...
          {!loading && !error && resultado && !naoRelacionado && (
            <button style={ov.btnPrimary} onClick={() => onConfirm(campos)}>
